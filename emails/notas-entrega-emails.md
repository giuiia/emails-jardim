...
- `imagens/` → os 6 banners novos (`email-banner-*.jpg`). **Precisam subir no repositório GitHub `LP-jardim-EarlyBooking`, numa pasta nova `emails/`** (mesmo repositório da LP, só que numa subpasta), antes de publicar os e-mails de verdade. Os HTMLs já apontam para `https://giuiia.github.io/LP-jardim-EarlyBooking/emails/...`.
